...
class TestSuccessfulHTTPSConnectivity(unittest.TestCase):
    ...
    def test_http_test(self):
        configurations = '{ "tests": [ {"name": "HTTP_CONNECTIVITY", "properties": { "address": "' + self.address + '", "method": "GET", "params" : null}} ] }'
        tests = UnitTestsHelpers.prepare_tests(configurations)
        self.assertEqual(len(tests), 1)
        self.assertEqual(tests[0].address, self.address)
    ...
    def test_https_test(self):
        configurations = '{ "tests": [ {"name": "HTTPS_CONNECTIVITY", "properties": { "address": "' + self.address + '", "method": "GET", "params" : null}} ] }'
        tests = UnitTestsHelpers.prepare_tests(configurations)
        self.assertEqual(len(tests), 1)
        self.assertEqual(tests[0].address, self.address)
    ...
    def test_multiple_tests(self):
        configurations = '{ "tests": [ {"name": "HTTPS_CONNECTIVITY", "properties": { "address": "' + self.address + '", "method": "GET", "params" : null}},' \
                                      '{"name": "HTTP_CONNECTIVITY", "properties": { "address": "' + self.address + '", "method": "GET", "params" : null}},' \
